...
const FORCE_TO_NUMBER = () => {
  const raw = String(process.env.TWILIO_FORCE_TO_NUMBER || "").trim();
  if (!raw) return "";
...
const sendWhatsApp = async ({ to, body, type = "MSG" }) => {
  ...
  const forced = FORCE_TO_NUMBER();
  const target = forced || to;
...
const sendWhatsAppTemplate = async ({ to, contentSid, contentVariables, type = "MSG" }) => {
  const client = getClient();
  const forced = FORCE_TO_NUMBER();
  const target = forced || to;
  const waTo   = toWhatsApp(target);
  ...
  if (!client) {
    logger(type, target, "⚠️ SKIPPED — Add Twilio credentials in .env", contentVariables);
    return { skipped: true, reason: "No Twilio credentials" };
  }

  if (!waTo) {
    logger(type, target, "❌ SKIPPED — Invalid phone number", contentVariables);
    return { skipped: true, reason: "Invalid phone" };
  }

  if (!contentSid) {
    logger(type, target, "❌ SKIPPED — Template ID not configured", contentVariables);
  ...
  try {
    ...
    logger(type, waTo, `✅ SENT (Template) — SID: ${msg.sid}`, contentVariables);
    return { success: true, sid: msg.sid, to: waTo };
  } catch (err) {
    logger(type, waTo, `❌ FAILED — ${err.message}`, contentVariables);
    return { success: false, error: err.message, to: waTo };
  }
};
...
// ════════════════════════════════════════════════════════════════════════════
// G. SIMPLE ORDER CONFIRMATION — Clean, minimal format for customers (TEMPLATE)
// ════════════════════════════════════════════════════════════════════════════
const sendSimpleOrderConfirmation = async ({
  phone, customerName, productName, address, paymentStatus = "Successful"
}) => {
  const templateId = process.env.TWILIO_TEMPLATE_ORDER_CONFIRMATION;
  
  // If template ID is not configured, fall back to text message
  if (!templateId) {
    const body = [
      `🛒 Nouveau Order Confirmation`,
      ``,
      `👤 Name: ${customerName}`,
      `📦 Product: ${productName}`,
      `📞 Phone: ${phone}`,
    ...
    return sendWhatsApp({ to: phone, body, type: "SIMPLE_ORDER_CONFIRM" });
  }
