  ...
  return (inrAmount) => {
    ...
    const currentCurrency =
      CURRENCY_BY_CODE[currencyCode] || CURRENCY_BY_CODE.INR;
    ...
    const converted =
      parsed * (Number(rates[currencyCode]) || exchangeRate || 1);
    ...
    return `${currentCurrency.symbol}${converted.toLocaleString(locale, {
...
const formatCurrency = useMemo(() => {
  return (amount) => {
    ...
    return `${currentCurrency.symbol}${parsed.toLocaleString(locale, {
      maximumFractionDigits: 2,
    })}`;
  };
}, [currencyCode]);
  ...
  () => ({
    ...
    detectedCurrencySymbol:
      (CURRENCY_BY_CODE[detectedCurrencyCode] || CURRENCY_BY_CODE.INR).symbol,
    setCurrencyCode,
    formatPrice,
    formatCurrency,
